...
'''
word2vec 使用的神经网络的输入是上下文，要预测（输出）的是被上下文包围的单词，
这跟英语的【完形填空】是一样的
'''
...
class SimpleCBOW:
    ...
    def forward(self, context, target):
        c0 = np.array(context[:, 0]) # 上文单词的 one-hot
        c1 = np.array(context[:, 1]) # 下文单词的 one-hot
...
if __name__ == '__main__':
    ...
    '''
    此 CBOW 结构如下：
      - 输入层（Matmul）大小：vocab_size * hidden_size，输入向量大小：hidden_size * vocab_size
      - 中间层（求平均）大小：vocab_size * hidden_size
      - 输出层（Matmul）大小：hidden_size * vocab_size，输入向量大小：vocab_size * hidden_size
    '''
